...
#
# Pocket ID client settings for Longhorn.
# Run from the core stack directory (repository root).
#
app_name=longhorn
client_name="Longhorn"
launch_url=$(pulumi stack output --json | jq -er '.storage.endpoints.longhorn')
launch_url="${launch_url%/}"
callback_urls=("$launch_url/oidc/callback")
logout_callback_urls=("$launch_url/oidc/callback")
dark_icon_url=https://cdn.jsdelivr.net/gh/selfhst/icons@main/svg/rancher-longhorn.svg
light_icon_url=https://cdn.jsdelivr.net/gh/selfhst/icons@main/svg/rancher-longhorn-light.svg
pkce_enabled=false
